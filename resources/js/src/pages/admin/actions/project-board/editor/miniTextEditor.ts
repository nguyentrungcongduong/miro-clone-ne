...
export function useDragMiniTextEditor() {
    ...
    function changeMiniTextEditorBodyContent(id: number) {
        ...
        if (!miniTextEditorContent) {
            setTimeout(() => changeMiniTextEditorBodyContent(id), 500);
            return;
        }
        ...
        function _changeMiniTextEditorContentOnKeyDownOrOnMouseUpEvent() {
            ...
            if (!blinkingCursor) return;
            blinkingCursor.style.display = "block";
            const cursorResult = getCursorPosition(miniTextEditorContent, blinkingCursor);
            if (!cursorResult) return;
            const { cursorPosition, x, y } = cursorResult;
    ...
    function dragMiniTextEditor(id: number) {
        ...
        if (!miniTextEditor || !miniTextEditorHandler || !miniTextEditorResizer) {
            setTimeout(() => dragMiniTextEditor(id), 500);
            return;
        }
